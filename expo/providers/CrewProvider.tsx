...
export const [CrewProvider, useCrew] = createContextHook(() => {
  ...
  const upcomingRides = useMemo(() =>
    rides
      // Active rides (happening right now) stay on the upcoming list so the
      // crew can still open them, check in, and use the ride chat.
      .filter((ride) => ride.status === 'upcoming' || ride.status === 'active')
